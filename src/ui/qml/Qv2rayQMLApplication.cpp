...
void Qv2rayQMLApplication::MessageBoxWarn(QWidget *parent, const QString &title,
                                          const QString &text) {
  QMessageBox::warning(parent, title, text);
}
void Qv2rayQMLApplication::MessageBoxInfo(QWidget *parent, const QString &title,
                                          const QString &text) {
  QMessageBox::information(parent, title, text);
}

MessageOpt Qv2rayQMLApplication::MessageBoxAsk(QWidget *parent,
                                               const QString &title,
                                               const QString &text,
                                               const QList<MessageOpt> &list) {
  QFlags<QMessageBox::StandardButton> btns;
  for (const auto &b : list) {
    btns.setFlag(MessageBoxButtonMap[b]);
  }
  return MessageBoxButtonMap.key(
      QMessageBox::question(parent, title, text, btns));
}

QStringList Qv2rayQMLApplication::checkPrerequisitesInternal() { return {}; }
...
Qv2rayExitReason Qv2rayQMLApplication::runQv2rayInternal() {
  ...
  connect(&engine, &QQmlApplicationEngine::objectCreated, this, connectLambda,
          Qt::QueuedConnection);
  ...
  node.usePreloadedDocument(applicationDirPath() + "/forms/MainWindow.qml",
                            window, warnings);
#endif
